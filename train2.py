...
#             save_visualization(generated_samples, (14,14), save_path='./temp_vis/sample_%04d.jpg' % int(iterations/step))

#         iterations += 1

#     save_path = saver.save(sess, "/home/cooper/GAN-MNIST/not-mnist/model.ckpt")
#     print("Model saved in file: %s" % save_path)

for idx in range(20):
    Z_np_sample = np.random.uniform(-1, 1, size=(visualize_dim,dim_z))
    Y_np_sample = OneHot( np.random.randint(10, size=[visualize_dim]))
    saver.restore(sess,'/home/cooper/GAN-MNIST/not-mnist/model.ckpt');
    generated_samples = sess.run(
                        image_tf_sample,
                        feed_dict={
                            Z_tf_sample:Z_np_sample,
    ...
    generated_samples = (generated_samples + 1.)/2.
    save_visualization(generated_samples,idx, Y_np_sample, (100,100), save_path='./vis2/sample_'+str(iterations/200)+'.jpg')