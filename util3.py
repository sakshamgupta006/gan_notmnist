...
def save_visualization(X,i, y_np_sample, nh_nw, save_path='./vis2/sample.jpg'):
    h,w = X.shape[1], X.shape[2]
    img = np.zeros((h * nh_nw[0], w * nh_nw[1], 3))
    K = X
    arr = K.reshape([K.shape[0],h*w])

    f = open('/home/cooper/GAN-MNIST/not-mnist/train_images' + str(i),'w+')
    np.save(f,arr)

    l = open('/home/cooper/GAN-MNIST/not-mnist/train_labels' + str(i) , 'w+')
    np.save(l, y_np_sample)
